...
class SO3FourierCoefficients:
    ...
    def __getitem__(self, index):
        if type(index[0]) == int:
            fhat_array_idx0 = self._fhat_array[self._dim(index[0] - 1):self._dim(index[0])].reshape(2 * index[0] + 1,
                                                                                                    2 * index[0] + 1)
...
class plan:
        ...
        for n in range(0, self._N_internal + 1):
            for k1 in range(-n, n + 1):
                for k2 in range(-n, n + 1):
                    self._coef_a[n, k1, k2] = n / ((n + 1) * (2 * n + 1)) * np.sqrt(
                        (np.power(n + 1, 2) - np.power(k1, 2)) * (np.power(n + 1, 2) - np.power(k2, 2)))
                    if n > 0:
                        self._coef_b[n, k1, k2] = (1. * k1 * k2) / (n * (n + 1.))
                        self._coef_c[n, k1, k2] = (n + 1.) / (n * (2 * n + 1.)) * np.sqrt(
                            (n * n - k1 * k1) * (n * n - k2 * k2))
    ...
    def _compute_Dtheta_matrix_multiplication(self, fhat):
        ...
        for n in range(0, fhat.N + 1):
            ...
            if n > 0:
                fhat_mult[n - 1, -n + 1:n - 1, -n + 1:n - 1] -= self._coef_c[n, -n + 1:n - 1, -n + 1:n - 1] * fhat[n,
                                                                                                              -n + 1:n - 1,
                                                                                                              -n + 1:n - 1]
        return fhat_mult
    ...
    def _compute_Dtheta_matrix_adjoint_multiplication(self, fhat):
        ...
        for n in range(0, fhat.N):
            ...
            fhat_mult[n, -n:n, -n:n] -= self._coef_b[n, -n:n, -n:n] * fhat[n, -n:n, -n:n]
            if n > 0:
                fhat_mult[n, -n + 1:n - 1, -n + 1:n - 1] -= self._coef_c[n, -n + 1:n - 1, -n + 1:n - 1] * fhat[n - 1,
                                                                                                          -n + 1:n - 1,
                                                                                                          -n + 1:n - 1]
        return fhat_mult
