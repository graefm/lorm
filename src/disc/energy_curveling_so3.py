...
class plan(ManifoldObjectiveFunction):
    def __init__(self, M, N, alpha, L, equality_constraint=False):
        ...
        self._nfsoft_plan = nfsoft.plan(M, N, self._m)
        ...
        def f(point_array_coords):
            ...
            return np.sum(np.real(np.dot(err_vec.array.conjugate(),
                                         self._lambda_hat.array * err_vec.array))) + self._alpha * 1. / self._M * np.sum(
                pos_diff_lengths ** 2)
        ...
        def grad(point_array_coords):
            ...
            grad = 2 * np.real(self._nfsoft_plan.compute_gradYmatrix_multiplication(
    ...
    def _eval_cos_signed_lengths_half(self, point_array_coords):
        ...
        return np.cos(dp[:, 0] / 2) * np.cos(dp[:, 2] / 2) * np.cos(dp[:, 1] / 2) - np.sin(dp[:, 0] / 2) * np.sin(
            dp[:, 2] / 2) * np.cos(st / 2)
    ...
    def _eval_grad_lengths1(self, point_array_coords):
        ...
        grad_lengths[:, 0] = (
                    -np.sin(dp[:, 0] / 2) * np.cos(dp[:, 2] / 2) * np.cos(dp[:, 1] / 2) - np.cos(dp[:, 0] / 2) * np.sin(
                dp[:, 2] / 2) * np.cos(st / 2))
        grad_lengths[:, 1] = (
                    -np.cos(dp[:, 0] / 2) * np.cos(dp[:, 2] / 2) * np.sin(dp[:, 1] / 2) + np.sin(dp[:, 0] / 2) * np.sin(
                dp[:, 2] / 2) * np.sin(st / 2))
        grad_lengths[:, 2] = (
                    -np.cos(dp[:, 0] / 2) * np.sin(dp[:, 2] / 2) * np.cos(dp[:, 1] / 2) - np.sin(dp[:, 0] / 2) * np.cos(
                dp[:, 2] / 2) * np.cos(st / 2))
