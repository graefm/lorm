...
class plan(ManifoldObjectiveFunction):
    def __init__(self, M, N, alpha, L, m=5, sigma=2, equality_constraint=False):
        ...
        self._m = m
        self._sigma = sigma
        self._nfdsft_plan = nfdsft.plan(M, N, m=self._m, sigma=self._sigma)
        ...
        def f(point_array_coords):
            ...
            return np.sum(np.real(np.dot(err_vector.array.conjugate(),
                                         self._lambda_hat.array * err_vector.array))) \
                   + self._alpha / self._M * np.sum(pos_diff_lengths ** 2)
        ...
        def grad(point_array_coords):
            ...
            return grad.reshape(2 * self._M, 2) + self._alpha / self._M * self._eval_grad_sum_lengths_squared(
                point_array_coords).reshape(2 * self._M, 2)
    ...
    def _eval_lengths(self, point_array_coords):
        ...
        lengths1_squared[1:self._M] = 2 * (
                1 - ct1[1:self._M] * ct1[0:self._M - 1] - np.cos(dp1[1:self._M]) * st1[1:self._M] * st1[0:self._M - 1])
        lengths2_squared[1:self._M] = 2 * (
                1 - ct2[1:self._M] * ct2[0:self._M - 1] - np.cos(dp2[1:self._M]) * st2[1:self._M] * st2[0:self._M - 1])
    ...
    def _eval_grad_lengths1(self, point_array_coords):
        ...
        grad_lengths[1:self._M, 0] = (st1[1:self._M] * ct1[0:self._M - 1]
                                      - np.cos(dp1[1:self._M]) * ct1[1:self._M] * st1[0:self._M - 1]) \
                                     / lengths[1:self._M]
        grad_lengths[1:self._M, 2] = (st2[1:self._M] * ct2[0:self._M - 1]
                                      - np.cos(dp2[1:self._M]) * ct2[1:self._M] * st2[0:self._M - 1]) \
                                     / lengths[1:self._M]
    ...
    def _eval_grad_lengths2(self, point_array_coords):
        ...
        lengths = self._eval_lengths(point_array_coords)
        grad_lengths = np.zeros([self._M, 4])
        grad_lengths[self._M - 1, 0] = (ct1[0] * st1[self._M - 1] - np.cos(dp1[0]) * st1[0] * ct1[self._M - 1]) \
                                       / lengths[0]
        grad_lengths[self._M - 1, 2] = (ct2[0] * st2[self._M - 1] - np.cos(dp2[0]) * st2[0] * ct2[self._M - 1]) \
                                       / lengths[0]
        ...
        grad_lengths[0:self._M - 1, 0] = (ct1[1:self._M] * st1[0:self._M - 1]
                                          - np.cos(dp1[1:self._M]) * st1[1:self._M] * ct1[0:self._M - 1]) \
                                         / lengths[1:self._M]
        grad_lengths[0:self._M - 1, 2] = (ct2[1:self._M] * st2[0:self._M - 1]
                                          - np.cos(dp2[1:self._M]) * st2[1:self._M] * ct2[0:self._M - 1]) \
                                         / lengths[1:self._M]
        grad_lengths[0:self._M - 1, 1] = -np.sin(dp1[1:self._M]) * st1[1:self._M] * st1[0:self._M - 1] \
                                         / lengths[1:self._M]
        grad_lengths[0:self._M - 1, 3] = -np.sin(dp2[1:self._M]) * st2[1:self._M] * st2[0:self._M - 1] \
                                         / lengths[1:self._M]
        return grad_lengths
