...
class plan(ManifoldObjectiveFunction):
    def __init__(self, M, N, alpha, L, equality_constraint=False, closed=True):
        ...
        def f(point_array_coords):
            ...
            return np.real(np.sum(
                err_vector.array * err_vector.array.conjugate() * self._lambda_hat.array)) + self._alpha * 1. / self._M * np.sum(
                (pos_diff_lengths) ** 2)
        ...
        def grad(point_array_coords):
            ...
            grad = 2 * np.real(self._nfsft_plan.compute_gradYmatrix_multiplication(
                le)) * self._weights + self._alpha * 1. / self._M * self._eval_grad_sum_lengths_squared(
                point_array_coords)
            return grad
    ...
    def _eval_lengths(self, point_array_coords):
        ...
        lengths[1:self._M] = np.sqrt(
            2 * (1 - ct[1:self._M] * ct[0:self._M - 1] - np.cos(dp[1:self._M]) * st[1:self._M] * st[0:self._M - 1]))
    ...
    def _eval_grad_lengths1(self, point_array_coords):
        ...
        grad_lengths[1:self._M, 0] = (st[1:self._M] * ct[0:self._M - 1]
                                      - np.cos(dp[1:self._M]) * ct[1:self._M] * st[0:self._M - 1]) / lengths[1:self._M]
    ...
    def _eval_grad_lengths2(self, point_array_coords):
        ...
        lengths = self._eval_lengths(point_array_coords)
        if self._closed:
            grad_lengths[self._M - 1, 0] = (ct[0] * st[self._M - 1] - np.cos(dp[0]) * st[0] * ct[self._M - 1]) \
                                           / lengths[0]
            grad_lengths[self._M - 1, 1] = -np.sin(dp[0]) * st[0] * st[self._M - 1] / lengths[0]
        grad_lengths[0:self._M - 1, 0] = (ct[1:self._M] * st[0:self._M - 1]
                                          - np.cos(dp[1:self._M]) * st[1:self._M] * ct[0:self._M - 1]) \
                                         / lengths[1:self._M]
