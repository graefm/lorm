...
class TangentVectorArray:
    ...
    def __str__(self):
        return f"Array of {len(self._coords)} tangent vectors of the manifold: " \
    ...
    def perform_geodesic_step(self, step_length=1):
        self._base_point_array._coords, self._coords = self.manifold.geodesic_step(self._base_point_array.coords,
                                                                                   self.coords, step=step_length)
...
class TangentVectorArrayParameterized(TangentVectorArray):
    ...
    def perform_geodesic_step(self, step_length=1):
        ...
        self._base_point_array._local_coords = self.manifold.compute_inverse_parameterization(
            self._base_point_array._coords)
        ...
        inverse_riemannian_matrix = self.manifold.compute_inverse_riemannian_matrix(
            self._base_point_array._local_coords)
...
class EuclideanSpace(Manifold):
    ...
    def compute_parameterization(self, local_coords):
        return np.array(local_coords)

    def compute_inverse_parameterization(self, coords):
        return np.array(coords)

    def compute_inverse_riemannian_matrix(self, local_coords):
        inv_g_matrix = np.zeros((local_coords.shape[0], self._local_dim, self._local_dim))
        for i in range(self._local_dim):
            inv_g_matrix[:, i, i] = 1
        return inv_g_matrix

    def compute_jacobi_matrix(self, local_coords):
        dh_matrix = np.zeros((local_coords.shape[0], self._dim, self._local_dim))
        for i in range(self._local_dim):
            dh_matrix[:, i, i] = 1
        return dh_matrix

    def compute_christoffel_matrix_lin_comb_parameterization(self, local_coords, coeffs):
        return np.zeros((local_coords.shape[0], self._local_dim, self._local_dim))


class ProductManifold(Manifold):
    def __init__(self, Manifolds, parameterized=True):
        self._Manifolds = Manifolds
        self._dims = [M._dim for M in Manifolds]
        self._dim = np.sum(self._dims)
        self._local_dims = [M._local_dim for M in Manifolds]
        self._local_dim = np.sum(self._local_dims)
        self._description = f"Product Manifold:"
        for M in Manifolds:
            self._description += "\n   " + M._description
        Manifold.__init__(self)
    ...
    def project_on_manifold(self, vector):
        p_vector = np.empty_like(vector)
        dim = 0
        for M in self._Manifolds:
            p_vector[:, dim:dim + M._dim] = M.project_on_manifold(vector[:, dim:dim + M._dim])
            dim += M._dim
        return p_vector
    ...
    def project_on_tangent_space(self, base_point_coords, vector):
        ...
        dim = 0
        for M in self._Manifolds:
            p_vector[:, dim:dim + M._dim] = \
                M.project_on_tangent_space(base_point_coords[:, dim:dim + M._dim], vector[:, dim:dim + M._dim])
            dim += M._dim
        return p_vector

    def geodesic_step(self, base_point_coords, tangent_vector_coords, step=1.0):
        new_base_point_coords = np.empty_like(base_point_coords)
        new_tangent_vector_coords = np.empty_like(tangent_vector_coords)
        dim = 0
        for M in self._Manifolds:
            new_base_point_coords[:, dim:dim + M._dim], new_tangent_vector_coords[:, dim:dim + M._dim] = \
                M.geodesic_step(base_point_coords[:, dim:dim + M._dim], tangent_vector_coords[:, dim:dim + M._dim],
                                step=step)
            dim += M._dim
        return new_base_point_coords, new_tangent_vector_coords

    def compute_parameterization(self, local_coords):
        point_coords = np.empty((local_coords.shape[0], self._dim))
        dim = 0
        local_dim = 0
        for M in self._Manifolds:
            point_coords[:, dim:dim + M._dim] = M.compute_parameterization(
                local_coords[:, local_dim:local_dim + M._local_dim])
            dim += M._dim
            local_dim += M._local_dim
        return point_coords

    def compute_inverse_parameterization(self, coords):
        local_coords = np.empty([coords.shape[0], self._local_dim])
        dim = 0
        local_dim = 0
        for M in self._Manifolds:
            local_coords[:, local_dim:local_dim + M._local_dim] = \
                M.compute_inverse_parameterization(coords[:, dim:dim + M._dim])
            dim += M._dim
            local_dim += M._local_dim
        return local_coords
    ...
    def compute_inverse_riemannian_matrix(self, local_coords):
        ...
        local_dim = 0
        for M in self._Manifolds:
            inv_g_matrix[:, local_dim:local_dim + M._local_dim, local_dim:local_dim + M._local_dim] = \
                M.compute_inverse_riemannian_matrix(local_coords[:, local_dim:local_dim + M._local_dim])
            local_dim += M._local_dim
        return inv_g_matrix

    def compute_jacobi_matrix(self, local_coords):
        jacobi_matrix = np.zeros((local_coords.shape[0], self._dim, self._local_dim))
        dim = 0
        local_dim = 0
        for M in self._Manifolds:
            jacobi_matrix[:, dim:dim + M._dim, local_dim:local_dim + M._local_dim] = \
                M.compute_jacobi_matrix(local_coords[:, local_dim:local_dim + M._local_dim])
            dim += M._dim
            local_dim += M._local_dim
        return jacobi_matrix

    def compute_christoffel_matrix_lin_comb_parameterization(self, local_coords, coeffs):
        christoffel_matrix = np.zeros((local_coords.shape[0], self._local_dim, self._local_dim))
        local_dim = 0
        for M in self._Manifolds:
            christoffel_matrix[:, local_dim:local_dim + M._local_dim, local_dim:local_dim + M._local_dim] = \
                M.compute_christoffel_matrix_lin_comb_parameterization(
                    local_coords[:, local_dim:local_dim + M._local_dim],
                    coeffs[:, local_dim:local_dim + M._local_dim])
            local_dim += M._local_dim
...
class Sphere(Manifold):
    ...
    def geodesic_step(self, base_point_coords, tangent_vector_coords, step=1.0):
        v_norm = np.sqrt(np.sum(tangent_vector_coords * tangent_vector_coords, axis=1)).reshape(
            [tangent_vector_coords.shape[0], 1])
        length = step * v_norm
        new_base_point_coords = np.cos(length) * base_point_coords + np.sin(length) * tangent_vector_coords / v_norm
        new_tangent_vector_coords = - v_norm * np.sin(length) * base_point_coords + np.cos(
            length) * tangent_vector_coords
        return new_base_point_coords, new_tangent_vector_coords
...
class Sphere2(Sphere):
    ...
    def compute_inverse_parameterization(point_coords):
        ...
        # phi
        local_coords[:, 1] = point_coords[:, 1] / (
                    point_coords[:, 0] - np.sqrt(point_coords[:, 0] ** 2 + point_coords[:, 1] ** 2))
...
class RotationGroup(MatrixManifold):
    ...
    def project_on_manifold(self, vector):
        ...
        U, _, V = np.linalg.svd(vector.reshape([vector_m, self._matrix_size[0], self._matrix_size[1]]),
                                full_matrices=True)
        m = np.matmul(U, V)
        det = np.linalg.det(m)
        for i in range(vector_m):
            if det[i] < 0:
                m[i, :, [0, 1]] = m[i, :, [1, 0]]
...
class GrassmannianStiefelRepresentation(MatrixManifold):
    def __init__(self, k, d):
        ...
        self._dim = k * d
        self._dim = int(k * (d - k))
        self._description = "Grassmannian G_{k_s},{d_s} in R^{d_r}".format(k_s=self._matrix_size[0],
                                                                           d_s=self._matrix_size[1], d_r=self._dim)
        Manifold.__init__(self)
