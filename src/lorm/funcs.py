...
class ManifoldObjectiveFunction:
    ...
    def grad(self,manifold_point_array):
        if not self._parameterized:
            ...
            inverse_riemannian_matrix = self.manifold.compute_inverse_riemannian_matrix(manifold_point_array.local_coords)
    ...
    def hess_mult(self,tangent_vector_array):
        ...
        if not self._parameterized:
            ...
            hess_mult_vector_array.coords = self._hess_mult(base_coords, tangent_vector_array.coords)\
                - tangent_vector_array.christoffel_matrix_lin_comb_mult(self._grad(base_coords))
            ...
            christoffel_matrix_lincomp_param = self.manifold.compute_christoffel_matrix_lin_comb_parameterization(base_local_coords,gradfp_local_coords)
            inverse_riemannian_matrix = self.manifold.compute_inverse_riemannian_matrix(base_local_coords)
            for i in range(self.manifold.local_dim):
                for j in range(self.manifold.local_dim):
                    hess_mult_local_coords[:,i] -= christoffel_matrix_lincomp_param[:,i,j]*tangent_local_coords[:,j]
...
class PotentialEnergyDotProductKernel(ManifoldObjectiveFunction):
    def __init__(self,manifold,kernel_func, parameterized=False):
        ...
        def hess_mult(point_array_coords,tangent_array_coords):
            return cfuncs.potential_energy_dot_product_kernel_hess_mult(kernel_func,point_array_coords,tangent_array_coords)

        ManifoldObjectiveFunction.__init__(self,manifold,f,grad=grad,hess_mult=hess_mult, parameterized=parameterized)
...
class PotentialEnergySquaredDistanceKernel(ManifoldObjectiveFunction):
    def __init__(self,manifold,kernel_func,parameterized=False):
        ...
        def hess_mult(point_array_coords,tangent_array_coords):
            return cfuncs.potential_energy_squared_distance_kernel_hess_mult(kernel_func,point_array_coords,tangent_array_coords)
...
class PotentialEnergyProjectiveDotProductKernel(ManifoldObjectiveFunction):
    def __init__(self,matrix_manifold,kernel_func,parameterized=False):
        ...
        def hess_mult(point_array_coords,tangent_array_coords):
            return cfuncs.potential_energy_projective_dot_product_kernel_hess_mult(self,kernel_func,point_array_coords,tangent_array_coords)
